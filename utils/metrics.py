...
def Q_index(x, y):
    ...
    #print(sigma_x, sigma_y, sigma_xy)
...
def D_lambda(fused_hsi, lr_hsi, band_list):
    ...
    fused_hsi_re = fused_hsi.reshape(N_spectral, -1)
    lr_hsi_re = lr_hsi.reshape(N_spectral, -1)
    ...
    for i in range(len(band_list)-1):
        for n in range(1, len(band_list)-i):
            ...
            index += 1
    #print(selected_bandnum, index, bands)
    #print(Q1, Q2)
...
def D_s(fused_hsi, lr_hsi, pan, down_ratio):
    ...
    #degraded_pan = F.interpolate(pan, scale_factor=(1/down_ratio, 1/down_ratio), mode='bilinear')
