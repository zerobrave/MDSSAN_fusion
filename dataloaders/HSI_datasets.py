...
class beijing_train_dataset(data.Dataset):
    def __init__(
        ...
        self.want_DHP_MS_HR = want_DHP_MS_HR  # This ask: DO we need DIP up-sampled output as dataloader output?
        self.is_dhp = is_dhp  # This checks: "Is this DIP training?"
        self.dir = self.config["beijing_train_dataset"]["data_dir"]  # Path to Pavia Center dataset
        ...
        self.augmentation = self.config["beijing_train_dataset"]["augmentation"]  # Augmentation needed or not?

        self.LR_crop_size = (self.config["beijing_train_dataset"]["LR_size"], self.config["beijing_train_dataset"]["LR_size"])  # Size of the the LR-HSI

        self.HR_crop_size = [self.config["beijing_train_dataset"]["HR_size"], self.config["beijing_train_dataset"]["HR_size"]]  # Size of the HR-HSI
...
class road_train_dataset(data.Dataset):
    def __init__(self, config, is_train=True, is_dhp=False, want_DHP_MS_HR=False):
        # Determine between train and val splits
        self.split = "seg_train" if is_train else "seg_val"
        ...
        self.dir = self.config["road_train_dataset"]["data_dir"]

        if self.split == "seg_val":
            self.file_list = os.path.join(self.dir, f"{self.split}" + ".txt")
        elif self.split == "seg_train":
            self.file_list = os.path.join(self.dir, f"{self.split}" + ".txt")
            if is_dhp:
                self.file_list = os.path.join(self.dir, f"{self.split}_dhp" + ".txt")
        ...
        # augmentations (Not applicable in this experiment actually)
        self.augmentation = self.config["road_train_dataset"]["augmentation"]

        # Reading the LR crop size
        self.LR_crop_size = (self.config["road_train_dataset"]["LR_size"], self.config["road_train_dataset"]["LR_size"])

        # High resolution crop size
        self.HR_crop_size = [self.config["road_train_dataset"]["HR_size"], self.config["road_train_dataset"]["HR_size"], ]
        ...
        for f in self.images:  #f为影像id
            self.files[self.split].append({"hrhsi": self.dir + 'hrhsi\\' + f + '.tif', "lrhsi": self.dir + 'lrhsi\\' + f + '.tif', "msi": self.dir + 'msi\\' +
                                                                                                                                         f + '.tif'})
    ...
    def getHSIdata(self, index):
        ...
        if self.want_DHP_MS_HR:
            opt_lambda = self.config["road_train_dataset"]["optimal_lambda"]
            mat_dhp = scipy.io.loadmat(image_dict["imgs"][:-4] + "_dhp_" + "{0:0=1d}".format(int(10 * opt_lambda)) + ".mat")
        ...
        MS_image = MS_image / self.config["road_train_dataset"]["max_value"]
        PAN_image = PAN_image / self.config["road_train_dataset"]["max_value"]
        reference = reference / self.config["road_train_dataset"]["max_value"]

        # If split = "train" and augment = "true" do augmentation
        if self.split == "fusion_train" and self.augmentation:
            MS_image, PAN_image, reference = self._augmentaion(MS_image, PAN_image, reference)
...
class dx_dataset(data.Dataset):
    def __init__(self, config, is_train=True, is_dhp=False, want_DHP_MS_HR=False):
        ...
        self.files = collections.defaultdict(list)
        for f in self.images:  #f为影像id
            self.files[self.split].append({"hrhsi": self.dir + 'ref\\' + f + '.tif', "lrhsi": self.dir + 'hsi\\' + f + '.tif', "msi": self.dir + 'rgb\\' +
                                                                                                                                         f + '.tif'})
...
class mdas_dataset(data.Dataset):
    def __init__(
            self, config, is_train=True, is_dhp=False, want_DHP_MS_HR=False
            ):
        # Settings
        ...
        # Paths
        self.dir = self.config["mdas_dataset"]["data_dir"]

        # Read train/val image indexes from the text file (train.txt, val.txt, train_dhp.txt)
        if self.split == "val":
            self.file_mat = os.path.join(self.dir, "MDAS_" + f"{self.split}" + ".mat")
        else:
            self.file_mat = os.path.join(self.dir, "MDAS_" + f"{self.split}" + ".mat")


        # list of all images
        self.images = scipy.io.loadmat(self.file_mat)

        # augmentations
        self.augmentation = self.config["mdas_dataset"]["augmentation"]

        self.LR_crop_size = (self.config["mdas_dataset"]["LR_size"], self.config["mdas_dataset"]["LR_size"])

        self.HR_crop_size = [
            self.config["mdas_dataset"]["HR_size"],
            self.config["mdas_dataset"]["HR_size"],
            ]
    ...
    def __len__(self):
        if self.split=='train':
            return len(self.images['hrhsi'])

        else:
            return len(self.images['val_hrhsi'])

    def getHSIdata(self, index):
        #mdas 数据路径为mat所在路径，先读mat，在读ref， msi，hsi，最后按index取
        ...
        if self.split=='train':
            ...
            img_key = 'val_'
        # read each image in list
        reference = self.images[img_key + "hrhsi"][index]
        PAN_image = self.images[img_key + "hrmsi"][index]
        MS_image = self.images[img_key+"lrhsi"][index]
        ...
        # Max Normalization
        MS_image = MS_image/self.config["mdas_dataset"]["max_value"]
        PAN_image = PAN_image/self.config["mdas_dataset"]["max_value"]
        reference = reference/self.config["mdas_dataset"]["max_value"]


        return str(index), MS_image, PAN_image, reference
